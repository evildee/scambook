...
search.prototype =
{
	...
	/* Latest Activity */
	/*LATEST_CHART_WIDTH: 560,
	LATEST_CHART_HEIGHT: 260,

	latest_graph_no_data: null,
	latest_graph_loader: null,
	latest_graph: null,*/

	/* Search Activiy */
	SEARCH_CHART_WIDTH: 300,
	SEARCH_CHART_HEIGHT: 230,

	search_graph_no_data: null,
	search_graph_loader: null,
	search_graph_list: null,
	search_graph: null,
	...
	binds: function()
	{
		...
		/*inst.latest_graph_no_data = $('#latest_graph_no_data');
		inst.latest_graph_loader = $('#latest_graph_loader');
		inst.latest_graph = $('#latest_graph');*/

		inst.search_graph_no_data = $('#search_graph_no_data');
		inst.search_graph_loader = $('#search_graph_loader');
		inst.search_graph_list = $('#search_graph_list');
		inst.search_graph = $('#search_graph');

		/*inst.latest_chart = new Highcharts.Chart({
			chart: {
				renderTo: 'latest_graph',
				type: 'line',
				width: inst.LATEST_CHART_WIDTH,
				height: inst.LATEST_CHART_HEIGHT
			},
			title: {
				text: ''
			},
		...
		inst.search_chart = new Highcharts.Chart({
			chart: {
				renderTo: 'search_graph',
				type: 'column',
				width: inst.SEARCH_CHART_WIDTH,
				height: inst.SEARCH_CHART_HEIGHT
			},
			title: {
				text: ''
			},
		...
		$('#day_intervals a').click(function() {
			...
			//inst.reloadLatestGraph();
			inst.reloadSearchGraph();
		});

		$('#search_graph_list li a').live('hover', function() {
			var index = $(this).attr('data-index');

			inst.search_chart.tooltip.refresh(inst.search_chart.series[0].data[index]);
		});
	},

	_init: function(companies)
	{
		this.companies = companies;

		this.binds();

		//this.reloadLatestGraph();
		this.reloadSearchGraph();
	},

	/*reloadLatestGraph: function()
	{
		var inst = this;

		inst.latest_graph.hide();
		inst.latest_graph_no_data.hide();
		inst.latest_graph_loader.show();

		inst.latest_chart.xAxis[0].options.tickInterval = (inst.day_interval == 30) ? 4 : null;

		$.getJSON('/insights/ajax_get_activity', { 'company_id': inst.companies, 'day_interval': inst.day_interval }, function(result) {
			...
			if (result.data.has_data)
			{
				...
				inst.latest_chart.xAxis[0].setCategories(categories, false);

				inst.latest_chart.series[0].setData(reported_damage, false);
				inst.latest_chart.series[1].setData(num_reports, false);
				inst.latest_chart.series[2].setData(num_searches);

				inst.latest_graph.show();
			}
			else
			{
				inst.latest_graph_no_data.show();
			}

			inst.latest_graph_loader.hide();
		});
	},*/

	reloadSearchGraph: function()
	{
		var inst = this;

		inst.search_graph_list.hide();
		inst.search_graph.hide();
		inst.search_graph_no_data.hide();
		inst.search_graph_loader.show();

		$.getJSON('/insights/ajax_get_search_activity', { 'company_id': inst.companies, 'day_interval': inst.day_interval }, function(result) {
			...
			if (num_searches > 0)
			{
				...
				inst.search_chart.series[0].setData(searches);

				inst.search_graph_list.html(result.data.search_activity_list_html);

				inst.search_graph_list.show();
				inst.search_graph.show();
			}
			else
			{
				inst.search_graph_no_data.show();
				inst.search_graph_list.hide();
				inst.search_graph.hide();
			}

			inst.search_graph_loader.hide();
		});
	}
}