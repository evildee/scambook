...
Media.prototype =
{
	...
	binds: function ()
	{
		...
		$('.media-update').live('click', function (event)
		{
			...
			if(update_type == 'delete')
			{
				$.alert({
					type: 'confirm',
					title: 'Confirm Report Media Deletion',
					text: '<p>Are you sure you want to delete report media #' + id + '?</p>',
					callback: function ()
					{
						$.post('/ajax_media_update/' + update_type + '/' + id, function (response)
						{
	...
	refresh_table: function (jqInst)
	{
		...
		if(!$sb.empty(page_number))
		{
			...
			inst.table_ajax_request = $.getJSON('/report_media/' + page_number,
				{
