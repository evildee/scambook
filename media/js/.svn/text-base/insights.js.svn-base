...
insights.prototype =
{
	...
	$pie_chart: null,
	pie_chart: null,
	$pie_chart_loader: null,
	PIE_CHART_REFRESH_INTERVAL: 60000,
	$last_updated_container: null,
	$last_updated_time: null,

	// Trending
	CHART_WIDTH: 385,
	CHART_HEIGHT: 200,
	latest_chart_width: 385,

	selected_tab: 'submission',
	selected_category: '',
	selected_day_interval: 30,
	...
	$next_comment: null,
	$comments: null,
	comments_sliding: false,
	COMMENTS_SLIDER_SPEED: 900, // Lower the faster

	binds: function()
	{
		var inst = this;

		inst.$pie_chart = $('#pie_chart');
		inst.$pie_chart_loader = $('#pie_chart_loader');
		inst.$last_updated_container = $('#last_updated_container');
		inst.$last_updated_time = $('#last_updated_time');

		Highcharts.getOptions().colors = Highcharts.map(Highcharts.getOptions().colors, function(color) {
			return {
				radialGradient: { cx: .5, cy: .3, r: .7 },
				stops: [
					[0, color],
					[1, Highcharts.Color(color).brighten(-.3).get('rgb')]
				]
			}
		});

		inst.pie_chart = new Highcharts.Chart({
			chart: { renderTo: 'pie_chart', width: 475, height: 320, backgroundColor: 'rgba(255, 255, 255, 0.1)', plotBackgroundColor: null, plotBorderWidth: null, plotShadow: null },
			title: { text: 'Most Popular Searches Right Now' },
			tooltip: { pointFormat: '<strong>{point.percentage}%</strong>', percentageDecimals: 1 },
			plotOptions: {
				pie: {
					dataLabels: {
						enabled: true,
						...
						distance: 20,
						formatter: function() {
							return '<a href="/search?search=' + encodeURIComponent(this.point.name) + '" target="_blank" style="font-weight:bold">' + this.point.name + '</a><br>' + Math.round(this.percentage) + '%';
						}
					}
				}
			},
			...
			inst.refreshPieChartInit(this, pie_chart_json, pie_chart_last_updated_time);
		});

		$('#day_interval').change(function(e) {
			inst.selected_day_interval = e.target.value;

			if (inst.selected_tab === 'submission')
			{
				inst.refreshSubmission();
			}
			else if (inst.selected_tab === 'search')
			{
				inst.refreshSearch();
			}
			else if (inst.selected_tab === 'state')
			{
				inst.refreshState();
			}
			else if (inst.selected_tab === 'category')
			{
				inst.refreshCategory();
			}
		});

		$('#category').change(function(e) {
			inst.selected_category = e.target.value;
		...
		$('#insights_heading_nav li').click(function() {
			var previous_tab = inst.selected_tab;
			inst.selected_tab = $(this).attr('data-tab');

			if (previous_tab === inst.selected_tab)
				return;

			// Hide previous
			$('#tabs .dropdown li').removeClass('active');
			$('#' + inst.selected_tab + '_tab_btn').addClass('active');

			if (previous_tab === 'submission')
			{
				inst.$submission_table.hide();
				inst.$submission_chart.hide();
			}
			else if (previous_tab === 'search')
			{
				inst.$search_table.hide();
				inst.$search_chart.hide();
			}
			else if (previous_tab === 'state')
			{
				inst.$state_table.hide();
				inst.$state_chart.hide();
			}
			else if (previous_tab === 'category')
			{
				inst.$category_table.hide();
				inst.$category_chart.hide();
			}
		...
		inst.$chart_loader = $('#chart_loader');
		//inst.$chart_no_data = $('#chart_no_data');

		inst.$table_loader = $('#table_loader');
		inst.$table_no_data = $('#table_no_data');

		inst.$chart_container = $('#chart_container');

		// Submission
		inst.$submission_table = $('#submission_table');
		inst.$submission_chart = $('#submission_chart');

		inst.submission_chart = new Highcharts.Chart({
			chart: { renderTo: 'submission_chart', type: 'column', width: inst.CHART_WIDTH, height: inst.CHART_HEIGHT, margin: [5, 0, 20, 40] },
			title: { text: '' },
			xAxis: { tickInterval: 1, tickPixelInterval: 2 },
			yAxis: { type: 'logarithmic', title: { text: '' } },
			tooltip: { formatter: function() {
				var hovered_index = this.series.data.indexOf(this.point);
				var company_title = $('#submission_table_body a[data-index=' + hovered_index + ']').html();

				return '<strong>' + company_title + '</strong><br>' + this.series.name + ': ' + this.y;
			} },
			plotOptions: { column: { pointWidth: 15, borderWidth: 0, groupPadding: .1, shadow: false } },
			series: [
				{
					name: 'Total Complaints',
					stack: 'total_complaints',
					data: [],
					pointWidth: 6,
					color: '#666',
					showInLegend: false
			...
			inst.refreshSubmissionInit(this, submission_json);
		});

		// Search
		inst.$search_table = $('#search_table');
		inst.$search_chart = $('#search_chart');

		inst.search_chart = new Highcharts.Chart({
			chart: { renderTo: 'search_chart', type: 'column', width: inst.CHART_WIDTH, height: inst.CHART_HEIGHT, margin: [5, 0, 20, 40] },
			title: { text: '' },
			xAxis: { tickInterval: 1, tickPixelInterval: 2 },
			yAxis: { type: 'logarithmic', title: { text: '' } },
			tooltip: { formatter: function() {
				var hovered_index = this.series.data.indexOf(this.point);
				var search_term = $('#search_table_body a[data-search_index=' + hovered_index + ']').html();

				return '<strong>' + search_term + '</strong><br>' + this.series.name + ': ' + this.y;
			} },
			plotOptions: { column: { pointWidth: 15, borderWidth: 0, groupPadding: 0.1 } },
			series: [
				{
					name: 'Total Searches',
					stack: 'total_searches',
					data: [],
					pointWidth: 6,
					color: '#666',
					showInLegend: false,
		...
		// State
		inst.$state_table = $('#state_table');
		inst.$state_chart = $('#state_chart');

		inst.state_chart = new Highcharts.Chart({
			chart: { renderTo: 'state_chart', type: 'column', width: inst.CHART_WIDTH, height: inst.CHART_HEIGHT, margin: [5, 0, 20, 40] },
			title: { text: '' },
			xAxis: { tickInterval: 1, tickPixelInterval: 2 },
			yAxis: { type: 'logarithmic', title: { text: '' } },
			tooltip: { formatter: function() {
				var hovered_index = this.series.data.indexOf(this.point);
				var state_name = $('#state_table_body a[data-index=' + hovered_index + ']').html();

				return '<strong>' + state_name + '</strong><br>' + this.series.name + ': ' + this.y;
			} },
			plotOptions: { column: { pointWidth: 15, borderWidth: 0, groupPadding: .1 } },
			series: [
				{
					name: 'Total Complaints',
					stack: 'total_states',
		...
		// Category
		inst.$category_table = $('#category_table');
		inst.$category_chart = $('#category_chart');

		inst.category_chart = new Highcharts.Chart({
			chart: { renderTo: 'category_chart', type: 'column', width: inst.CHART_WIDTH, height: inst.CHART_HEIGHT, margin: [5, 0, 20, 40] },
			title: { text: '' },
			xAxis: { tickInterval: 1, tickPixelInterval: 2 },
			yAxis: { type: 'logarithmic', title: { text: '' } },
			tooltip: { formatter: function() {
				var hovered_index = this.series.data.indexOf(this.point);
				var category_name = $('#category_table_body a[data-index=' + hovered_index + ']').html();

				return '<strong>' + category_name + '</strong><br>' + this.series.name + ': ' + this.y;
			} },
			plotOptions: { column: { pointWidth: 15, borderWidth: 0, groupPadding: .1 }	},
			series: [
				{
					name: 'Total Complaints',
					stack: 'total_categories',
					data: [],
					pointWidth: 6,
					color: '#666',
					showInLegend: false,
		...
		// When scrolling chart
		inst.$chart_container.scroll(function() {
			inst.is_chart_scrolling = true;

			$('.table-body').scrollTop($(this).scrollLeft() * inst.scroll_ratio_x);
		});

		// Comments
		inst.$comments = $('#comments');

		inst.$prev_comment = $('#prev_comment a');
		inst.$next_comment = $('#next_comment a');

		inst.$prev_comment.live('click', function() {
			inst.getPreviousComments();
		});

		inst.$next_comment.live('click', function() {
			inst.getNextComments();
		});

		// Start pie chart timer
		setInterval((function(self) {
			return function() { self.refreshPieChart(); }
		})(this), inst.PIE_CHART_REFRESH_INTERVAL);
	},
	...
	refreshPieChart: function() {
		...
		inst.$pie_chart.hide();
		inst.$last_updated_container.hide();
		inst.$pie_chart_loader.show();

		// AJAX
		$.getJSON('/insights/ajax_get_pie_chart', {}, function(result) {
			var chart_data = [];

			var num_popular_searches = result.data.popular_searches.length;

			if (num_popular_searches > 0)
			{
				// Add result to chart
				//chart_data.push([ 'Other', parseInt(result.data.num_other_searches) ]);

				for (var i = 0; i < num_popular_searches; i++)
					chart_data.push([ result.data.popular_searches[i].term, parseInt(result.data.popular_searches[i].num_searches) ]);

				if (inst.pie_chart) {
					inst.pie_chart.series[0].setData(chart_data, true);
				}

				inst.$last_updated_time.html(result.data.last_updated_time);

				inst.$pie_chart_loader.hide();
				inst.$pie_chart.show();
				inst.$last_updated_container.show();
			}
			else
			{
				inst.$pie_chart_loader.hide();
	...
	refreshPieChartInit: function(chart, popular_searches, last_updated_time)
	{
		var inst = this;
		...
		var num_popular_searches = popular_searches.length;
		...
		if (num_popular_searches > 0)
		{
			...
			for (var i = 0; i < num_popular_searches; i++)
				chart_data.push([ popular_searches[i].term, parseInt(popular_searches[i].num_searches) ]);

			if (chart) {
				chart.series[0].setData(chart_data, true);
			}

			inst.$last_updated_time.html(last_updated_time);
	...
	loadCategories: function(type)
	{
		...
		$.getJSON('/insights/ajax_get_categories', { 'type': type, 'day_interval': inst.selected_day_interval }, function(result) {
			inst.$category.html(result.html);
		});
	},

	resetScrollBars: function()
	{
		this.$chart_container.scrollLeft(0);
		$('.table-body').scrollTop(0);
	},

	updateScrollRatio: function(chart_width, table_height)
	{
		var inst = this;

		var ratio_diff = .496; // .496

		inst.scroll_ratio_x = chart_width / table_height;
		inst.scroll_ratio_x -= ratio_diff;

		inst.scroll_ratio_y = table_height / chart_width;
		inst.scroll_ratio_y += ratio_diff;

		// Assign event when scrolling table
		$('#' + inst.selected_tab + '_table_body').scroll(function() {
			// Scroll chart also
			if (!inst.is_chart_scrolling)
				inst.$chart_container.scrollLeft($(this).scrollTop() * inst.scroll_ratio_y);

			inst.is_chart_scrolling = false;
		});
	},

	refreshSubmission: function(reload_categories)
	{
		var inst = this;

		// Load categories
		if (reload_categories === true)
			inst.loadCategories('submission');

		// Reset scrollsbars
		inst.resetScrollBars();

		inst.$submission_chart.hide();
		inst.$submission_table.hide();
		inst.$table_no_data.hide();
		inst.$chart_loader.show();
		inst.$table_loader.show();

		$.getJSON('/insights/ajax_get_submission', { day_interval: inst.selected_day_interval, category: inst.selected_category }, function(result) {
			var categories = [], company_current = [], company_total = [];

			var num_companies = result.data.companies.length;
			...
			if (num_companies > 0)
			{
				...
				{
					var company = result.data.companies[i];

					categories.push(i + 1);

					company_current.push(parseInt(company.num_reports_this_period));
					company_total.push(parseInt(company.num_total_reports));
				}

				// Add categories
				inst.submission_chart.xAxis[0].setCategories(categories, false);

				// Add data
				inst.submission_chart.series[0].setData(company_total, false);
				inst.submission_chart.series[1].setData(company_current);

				// Resize chart depending on number of companies
				var chart_width = inst.CHART_WIDTH * (num_companies / 10);
				inst.submission_chart.setSize(chart_width, inst.CHART_HEIGHT, false);
				inst.latest_chart_width = chart_width;

				inst.$submission_chart.show();
				inst.$submission_table.html(result.data.html).show();
				inst.$chart_container.show();

				inst.updateScrollRatio(chart_width, $('#submission_table .table-body')[0].scrollHeight);
			}
			else
			{
				inst.$chart_container.hide();
				inst.$table_no_data.show();
			}

			inst.$chart_loader.hide();
			inst.$table_loader.hide();
		});
	},

	refreshSubmissionInit: function(chart, companies)
	{
		var inst = this;
		...
		if (num_companies > 0)
		{
			...
			// Add data
			chart.series[0].setData(company_total, false);
			chart.series[1].setData(company_current);

			// Resize chart depending on number of companies
			var chart_width = inst.CHART_WIDTH * (num_companies / 10);
			chart.setSize(chart_width, inst.CHART_HEIGHT, false);
			inst.latest_chart_width = chart_width;

			inst.$submission_chart.show();
			inst.$submission_table.html($('#submission_data').html()).show();
			inst.$chart_container.show();
	...
	refreshSearch: function()
	{
		...
		inst.$search_chart.hide();
		inst.$search_table.hide();
		//inst.$chart_no_data.hide();
		inst.$table_no_data.hide();
		inst.$chart_loader.show();
		inst.$table_loader.show();

		$.getJSON('/insights/ajax_get_search', { day_interval: inst.selected_day_interval }, function(result) {
			var categories = [], search_current = [], search_total = [];

			var num_searches = result.data.searches.length;
			...
			if (num_searches > 0)
			{
				...
				{
					var search = result.data.searches[i];

					categories.push(i + 1);

					search_current.push(parseInt(search.num_searches_this_period));
					search_total.push(parseInt(search.num_total_searches));
				}

				// Add categories
				inst.search_chart.xAxis[0].setCategories(categories, false);

				// Add data
				inst.search_chart.series[0].setData(search_total, false);
				inst.search_chart.series[1].setData(search_current);

				// Resize chart depending on number of companies
				var chart_width = inst.CHART_WIDTH * (num_searches / 10);
				inst.search_chart.setSize(chart_width, inst.CHART_HEIGHT, false);
				inst.latest_chart_width = chart_width;

				inst.$search_chart.show();
				inst.$search_table.html(result.data.html).show();
				inst.$chart_container.show();

				inst.updateScrollRatio(chart_width, $('#search_table .table-body')[0].scrollHeight);
			}
			else
			{
				inst.$chart_container.hide();
				//inst.$chart_no_data.show();
				inst.$table_no_data.show();
			}
	...
	refreshState: function(reload_categories)
	{
		var inst = this;
		...
		$.getJSON('/insights/ajax_get_state', { day_interval: inst.selected_day_interval, category: inst.selected_category }, function(result) {
			var categories = [], report_current = [], report_total = [];
			var num_states = result.data.states.length;
			...
			if (num_states > 0)
			{
				...
				{
					var state = result.data.states[i];

					categories.push(i + 1);

					report_current.push(parseInt(state.num_reports_this_period));
					report_total.push(parseInt(state.num_total_reports));
				}

				// Add categories
				inst.state_chart.xAxis[0].setCategories(categories, false);

				// Add data
				inst.state_chart.series[0].setData(report_total, false);
				inst.state_chart.series[1].setData(report_current);

				// Resize chart depending on number of companies
				var chart_width = inst.CHART_WIDTH * (num_states / 10);
				inst.state_chart.setSize(chart_width, inst.CHART_HEIGHT, false);
				inst.latest_chart_width = chart_width;

				inst.$state_chart.show();
				inst.$state_table.html(result.data.html).show();
				inst.$chart_container.show();

				inst.updateScrollRatio(chart_width, $('#state_table .table-body')[0].scrollHeight);
			}
			else
			{
				inst.$chart_container.hide();
	...
	refreshCategory: function()
	{
		...
		$.getJSON('/insights/ajax_get_category', { day_interval: inst.selected_day_interval }, function(result) {
			var categories = [], report_current = [], report_total = [];
			var num_categories = result.data.categories.length;
			...
			if (num_categories > 0)
			{
				...
				for (var i = 0; i < num_categories; i++)
				{
					...
					report_current.push(parseInt(category.num_reports_this_period));
					report_total.push(parseInt(category.num_total_reports));
				}

				// Add categories
				inst.category_chart.xAxis[0].setCategories(categories, false);

				// Add data
				inst.category_chart.series[0].setData(report_total, false);
				inst.category_chart.series[1].setData(report_current);

				// Resize chart depending on number of companies
				var chart_width = inst.CHART_WIDTH * (num_categories / 10);
				inst.category_chart.setSize(chart_width, inst.CHART_HEIGHT, false);
				inst.latest_chart_width = chart_width;

				inst.$category_chart.show();
				inst.$category_table.html(result.data.html).show();
				inst.$chart_container.show();

				inst.updateScrollRatio(chart_width, $('#category_table .table-body')[0].scrollHeight);
			}
			else
			{
				inst.$chart_container.hide();
	...
	/*_getPreviousComments: function()
	{
		var inst = this;

		if (inst.comments_sliding)
			return;

		inst.comments_offset -= 6;

		// Animate
		inst.comments_sliding = true;

		// First, get new comments
		$.getJSON('/insights/get_comments', { from: 'prev', offset: inst.comments_offset }, function(result) {
			// Prepend new comments to div
			inst.$comments.prepend(result.html);

			// Before animate, change position
			inst.$comments.css('left', '-780px');

			// Hide next arrow if end
			if (inst.comments_offset === 0)
				inst.$prev_comment.css('visibility', 'hidden');

			// Animate to right
			inst.$comments.animate({
				left: '+=780px'
			}, inst.COMMENTS_SLIDER_SPEED, function() {
				// Done sliding

				// Keep markup clean
				$('#comments li:gt(2)').remove();
				inst.$comments.css('left', '0');

				// Show prev arrow
				inst.$next_comment.css('visibility', 'visible');

				inst.comments_sliding = false;
			});
		});
	},

	_getNextComments: function()
	{
		var inst = this;
		...
		// First, get new comments
		$.getJSON('/insights/get_comments', { from: 'next', offset: inst.comments_offset }, function(result) {
			// Append new comments to div
			inst.$comments.append(result.html);

			// Hide next arrow if end
			if (inst.comments_offset > 24)
				inst.$next_comment.css('visibility', 'hidden');
			...
			inst.$comments.animate({
				...
				// Keep markup clean
				$('#comments li:lt(3)').remove();
				inst.$comments.css('left', '0');

				// Show prev arrow
				inst.$prev_comment.css('visibility', 'visible');
	...
	getPreviousComments: function()
	{
		...
		inst.$comments.animate({
			left: '+=780px'
		}, inst.COMMENTS_SLIDER_SPEED, function() { // Done sliding
			// Show prev arrow
			inst.$next_comment.css('visibility', 'visible');
	...
	resetChartSize: function()
	{
		...
		if (inst.selected_tab === 'submission') {
			inst.submission_chart.setSize(inst.latest_chart_width, inst.CHART_HEIGHT, false);
		} else if (inst.selected_tab === 'search') {
			inst.search_chart.setSize(inst.latest_chart_width, inst.CHART_HEIGHT, false);
		} else if (inst.selected_tab === 'state') {
			inst.state_chart.setSize(inst.latest_chart_width, inst.CHART_HEIGHT, false);
		} else if (inst.selected_tab === 'category') {
			inst.category_chart.setSize(inst.latest_chart_width, inst.CHART_HEIGHT, false);
		}
	}
}

function insights_company() {};

insights_company.prototype = {
	company_id: 0,

	uri: '',

	day_interval: 30,
	day_interval_info: null,

	activity_chart: null,
	search_activity_chart: null,

	binds: function()
	{
		var inst = this;

		inst.day_interval_info = $('#day_interval_info');

		$('#day_intervals a').click(function() {
			var $this = $(this);

			var day_interval = $this.attr('data-day_interval');

			// Abort if clicked on already selected item
			if (day_interval == inst.day_interval)
				return;

			$('#day_intervals a').removeClass('active');
			$this.addClass('active');

			inst.day_interval_info.html('For ' + day_interval + ' day' + (day_interval != 1 ? 's' : ''));

			inst.day_interval = day_interval;

			inst.refreshActivityGraph();
			inst.refreshSearchActivityGraph();
		});

		$('#search_activity_list li a').live('hover', function() {
			var index = $(this).attr('data-index');

			inst.search_activity_chart.tooltip.refresh(inst.search_activity_chart.series[0].data[index]);
		});

		inst.activity_chart = new Highcharts.Chart({
			chart: {
		...
		inst.search_activity_chart = new Highcharts.Chart({
			chart: {
				renderTo: 'search_activity_chart',
				type: 'column',
				width: 320,
				height: 260
			},
	...
	_init: function(company_id)
	{
		...
		if (this.uri !== '')
		{
			$.getJSON('/ajax/ajax_get_page_views', { 'uri': this.uri, 'id': this.id }, function(result) {
				$('#company_page_views').html(result.html);
			});
		}

		this.refreshActivityGraph();
		this.refreshSearchActivityGraph();
	},

	refreshActivityGraph: function()
	{
		var inst = this;

		$('#activity_chart, #activity_chart_container .no-data').hide();
		$('#activity_chart_container .loader').show();

		$.getJSON('/insights/ajax_get_activity', { 'company_id': inst.company_id, 'day_interval': inst.day_interval }, function(result) {
			var categories = [], num_reports = [], num_searches = [], reported_damage = [];

			if (result.data.has_data)
			{
				$.each(result.data.activities, function(date, obj) {
					categories.push(obj.formatted_date);

					reported_damage.push(obj.reported_damage);
					num_reports.push(obj.num_reports);
					num_searches.push(obj.num_searches);
				});

				if (inst.day_interval == 30)
				{
					inst.activity_chart.xAxis[0].options.tickInterval = 4;
				}
				else
				{
					inst.activity_chart.xAxis[0].options.tickInterval = null;
				}

				inst.activity_chart.xAxis[0].setCategories(categories, false);

				inst.activity_chart.series[0].setData(reported_damage, false);
				inst.activity_chart.series[1].setData(num_reports, false);
				inst.activity_chart.series[2].setData(num_searches);

				$('#activity_chart_container .loader').hide();
				$('#activity_chart').show();
			}
			else
			{
				$('#activity_chart_container .loader').hide();
				$('#activity_chart_container .no-data').show();
			}
		});
	},

	refreshSearchActivityGraph: function()
	{
		var inst = this;

		$('#search_activity_list, #search_activity_chart_container, #search_activity_container .no-data').hide();
		$('#search_activity_container .loader').show();

		$.getJSON('/insights/ajax_get_search_activity', { 'company_id': inst.company_id, 'day_interval': inst.day_interval }, function(result) {
			var categories = [], searches = [];
			...
			if (num_searches > 0)
			{
				for (var i = 0; i < num_searches; i++)
				{
					...
					searches.push(result.data.searches[i].num_searches);
				}

				inst.search_activity_chart.xAxis[0].setCategories(categories, false);

				inst.search_activity_chart.series[0].setData(searches);

				$('#search_activity_list').html(result.data.search_activity_list_html);

				$('#search_activity_container .loader').hide();
				$('#search_activity_list, #search_activity_chart_container').show();
			}
			else
			{
				$('#search_activity_container .loader').hide();
				$('#search_activity_container .no-data').show();
			}
		});
	}
}